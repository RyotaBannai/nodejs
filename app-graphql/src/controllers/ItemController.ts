...
export class ItemController {
  ...
  async addToList(@Body() data: AddType) {
    // run query and check if the data already exists, if so, return at the beginning.
    const this_item: Item = await this.repository.findOneOrFail(data.id);
    console.log(this_item);
    if (getManager().hasId(this_item)) {
      console.log(getManager().getId(this_item));
    }
    ...
    if (data.type === "list") {
      ...
      item_list.list = list;
      await getRepository(ItemList)
        .save(item_list)
        .then(() => console.log("saved"))
        .catch(() => {
          console.log("error on save: the relation may already exists.");
          return;
        });
